...
from .Resnet50 import model_resnet50, model_resnet18
from .ClassicalModel import SobelEdgeDetector


class model_resnet18_4(nn.Module):
    def __init__(self, num_keypoint=133, weights=None):
        super(model_resnet18_4, self).__init__()
        ...
        self.encoder.fc = nn.Linear(512, 133 * 3)
        self.relu = nn.ReLU()
        self.fc1 = nn.Linear(7 * 7 * 128 * 4, 1024)
...
class model_resnet18_5(nn.Module):
    def __init__(self, num_keypoint=133, weights=None):
        super(model_resnet18_5, self).__init__()
        self.encoder = models.resnet18(weights=weights)
...
class model_resnet50_5(nn.Module):
    def __init__(self, num_keypoint=133, weights=None):
        super(model_resnet50_5, self).__init__()
        self.encoder = models.resnet50(weights=weights)
...
class model_resnet50_with_sift(nn.Module):   
    ...
    def forward(self, x, sift_features):
        ...
        return output
class model_resnet18_with_sift(nn.Module):
    def __init__(self, weights=None, num_keypoints=133, num_sift_features=128):
        super(model_resnet18_with_sift, self).__init__()
        
        self.resnet = model_resnet18(weights=weights)
        # Define a processing layer for SIFT features
        self.sift_processor = nn.Sequential(
            nn.Linear(num_keypoints * num_sift_features, 1024),
            nn.ReLU(),
...
class model_resnet18_4_with_sobel(nn.Module):
    def __init__(self, weights=None):
        super(model_resnet18_4_with_sobel, self).__init__()
...
class model_resnet50_5_with_sobel_superpixel(nn.Module):
    def __init__(self, weights=None):
        ...
        self.sobel = SobelEdgeDetector()  # (N, 1, 224, 224)
        self.resnet = model_resnet50_5(weights=weights)  # (N, 133*3)

    def forward(self, x, superpixel_labels):
        edges = self.sobel(x)
        x = torch.cat((x, edges, superpixel_labels), dim=1)
        x = self.resnet(x)
        return x
    
class model_resnet18_5_with_sobel_superpixel(nn.Module):
    def __init__(self, weights=None):
...
if __name__ == "__main__":
    ...
    net = model_resnet50_5_with_sobel_superpixel().to(device)
    batch_size = 1
    a = torch.rand(batch_size, 3, 224, 224).to(device)
    a_= torch.rand(batch_size, 1, 224, 224).to(device)
    b = net(a,a_)
    print(b.shape)

    from thop import profile

    flops, params = profile(net, inputs=(a,a_), verbose=False)
